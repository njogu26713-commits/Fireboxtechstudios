  ...
  if (isLoading) {
    return (
      <div className="container mx-auto px-4 py-24 animate-pulse">
        ...
        <div className="aspect-video bg-white/10 rounded-3xl w-full mb-8"></div>
  ...
  return (
    <div className="pb-24">
      <div className="container mx-auto px-4 md:px-6 pt-12">
        <Link href="/tutorials" className="inline-flex items-center gap-2 text-white/50 hover:text-white mb-8 transition-colors">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-4 mb-4 text-sm font-mono">
            ...
            {tutorial.difficulty && <span className="flex items-center gap-1 text-white/60"><BarChart size={14}/> {tutorial.difficulty}</span>}
            {tutorial.duration && <span className="flex items-center gap-1 text-white/60"><Clock size={14}/> {tutorial.duration}</span>}
          </div>
          
          <h1 className="text-4xl md:text-5xl font-display font-bold mb-8">{tutorial.title}</h1>

          {embedUrl ? (
            <div className="aspect-video rounded-3xl overflow-hidden border border-white/10 mb-12 shadow-2xl bg-black">
              <iframe 
                src={embedUrl} 
            ...
            <div className="aspect-video rounded-3xl border border-white/10 mb-12 bg-white/5 flex items-center justify-center text-white/30">
              <Play size={48} />
          ...
          <div className="glass-panel p-8 rounded-3xl prose prose-invert max-w-none mb-12">
            ...
            <p className="text-white/80 whitespace-pre-wrap">{tutorial.description}</p>
          </div>
          ...
          {tutorial.downloadableResources && (
            <div className="glass-panel-glow p-8 rounded-3xl flex items-center justify-between">
              <div>
                ...
                <p className="text-white/60 text-sm">Download the source code and assets used in this tutorial.</p>
              </div>
