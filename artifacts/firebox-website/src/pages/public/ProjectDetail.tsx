    ...
    return (
      <div className="container mx-auto px-4 py-24">
        <div className="animate-pulse max-w-4xl mx-auto space-y-8">
          ...
          <div className="h-[500px] bg-white/10 rounded-3xl w-full"></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="pb-24">
      <div className="container mx-auto px-4 md:px-6 pt-12">
        <Link href="/portfolio" className="inline-flex items-center gap-2 text-white/50 hover:text-white mb-8 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            {screenshots.length > 0 && (
              <div className="space-y-8">
                {screenshots.map((url, i) => (
                  <img key={i} src={url} alt={`Screenshot ${i + 1}`} className="w-full rounded-3xl border border-white/10 shadow-2xl" />
                ))}
              </div>
            )}
            
            {project.videoUrl && (
              <div className="mt-12 aspect-video rounded-3xl overflow-hidden border border-white/10">
          ...
          <div className="space-y-8">
            <div className="glass-panel p-8 rounded-3xl">
              <h3 className="text-xl font-display font-bold mb-6 border-b border-white/10 pb-4">Project Details</h3>
              
              <div className="space-y-6 text-sm">
                {project.category && (
                  <div>
                    <div className="text-white/50 mb-1 flex items-center gap-2"><Layers size={14} /> Category</div>
                    <div className="font-medium text-white/90">{project.category}</div>
                  </div>
                )}
                
                <div>
                  <div className="text-white/50 mb-2 flex items-center gap-2"><Tag size={14} /> Technologies</div>
                  <div className="flex flex-wrap gap-2">
                    {techs.map(tech => (
                      <span key={tech} className="px-3 py-1 rounded bg-white/5 border border-white/10 text-white/80">{tech}</span>
                    ))}
                  </div>
                </div>
                ...
                <div className="pt-6 mt-6 border-t border-white/10 flex flex-col gap-3">
                  ...
                  {project.githubUrl && (
                    <a href={project.githubUrl} target="_blank" rel="noreferrer" className="w-full h-12 rounded-xl bg-white/10 text-white font-bold flex items-center justify-center gap-2 hover:bg-white/20 transition-all">
