    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="space-y-4">
        {isLoading ? <div className="p-8">Loading...</div> : 
         messages.length === 0 ? <div className="p-8 glass-panel rounded-2xl text-white/50">No messages.</div> :
         messages.map(msg => (
           <div key={msg.id} className={`glass-panel p-6 rounded-2xl border-l-4 ${msg.read ? 'border-l-white/10 opacity-70' : 'border-l-primary'}`}>
             <div className="flex justify-between items-start mb-4">
               <div>
                 ...
                 <div className="text-sm text-white/60">From: {msg.name} ({msg.email}) • {msg.phone || 'No phone'}</div>
               </div>
               <div className="text-sm text-white/40">{new Date(msg.createdAt).toLocaleString()}</div>
             </div>
             <p className="text-white/80 whitespace-pre-wrap mb-4 bg-black/20 p-4 rounded-lg">{msg.message}</p>
             <div className="flex justify-end gap-2">
               <button onClick={() => handleMarkRead(msg.id, !msg.read)} className="px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded text-sm flex items-center gap-1">
                 {msg.read ? 'Mark Unread' : <><Check size={14}/> Mark Read</>}
               </button>
               <button onClick={() => handleDelete(msg.id)} className="px-3 py-1.5 bg-destructive/20 text-destructive hover:bg-destructive hover:text-white rounded text-sm flex items-center gap-1">
