    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-white/10 pb-4">
        {(['faq', 'clients'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors capitalize ${tab === t ? 'bg-purple-600 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}>
      ...
      {tab === 'faq' && (
        <div className="space-y-4">
          ...
          {faqLoading ? <div className="text-white/60">Loading...</div> : (
                ...
                <div key={f.id} className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-white/50 mt-1 line-clamp-2">{f.answer}</div>
                      {f.category && <span className="text-xs text-purple-400 mt-1 inline-block">{f.category}</span>}
                    </div>
                    <div className="flex gap-2 flex-shrink-0">
                      <button onClick={() => { setFaqForm({ question: f.question, answer: f.answer, category: f.category || '', sortOrder: f.sortOrder || 0 }); setEditingId(f.id); setIsModalOpen(true); }} className="p-2 hover:bg-white/10 rounded-lg text-blue-400"><Edit2 size={16} /></button>
                      <button onClick={() => { if (!confirm('Delete FAQ?')) return; deleteFaq.mutate({ id: f.id }, { onSuccess: () => queryClient.invalidateQueries({ queryKey: ['listFaqItems'] }) }); }} className="p-2 hover:bg-white/10 rounded-lg text-red-400"><Trash2 size={16} /></button>
                    </div>
                  </div>
                </div>
              ))}
              {(faqs as any[]).length === 0 && <div className="p-8 text-center text-white/40">No FAQ items yet.</div>}
            </div>
          )}
        </div>
      )}
      ...
      {tab === 'clients' && (
        <div className="space-y-4">
          ...
          {clientLoading ? <div className="text-white/60">Loading...</div> : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {(clients as any[]).map((c: any) => (
                <div key={c.id} className="bg-white/5 border border-white/10 rounded-xl p-4 flex items-center gap-4">
                  {c.logoUrl && <img src={c.logoUrl} alt={c.name} className="w-12 h-12 object-contain rounded-lg bg-white/5 p-1" />}
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => { setClientForm({ name: c.name, logoUrl: c.logoUrl || '', websiteUrl: c.websiteUrl || '', sortOrder: c.sortOrder || 0 }); setEditingId(c.id); setIsModalOpen(true); }} className="p-2 hover:bg-white/10 rounded-lg text-blue-400"><Edit2 size={16} /></button>
                    <button onClick={() => { if (!confirm('Delete client?')) return; deleteClient.mutate({ id: c.id }, { onSuccess: () => queryClient.invalidateQueries({ queryKey: ['listTrustedClients'] }) }); }} className="p-2 hover:bg-white/10 rounded-lg text-red-400"><Trash2 size={16} /></button>
                  </div>
                </div>
              ))}
              {(clients as any[]).length === 0 && <div className="col-span-full p-8 text-center text-white/40">No trusted clients yet.</div>}
            </div>
          )}
        </div>
      )}
            ...
            {tab === 'faq' ? (
              <form onSubmit={handleFaqSubmit} className="space-y-4">
                {[{ key: 'question', label: 'Question', required: true }, { key: 'category', label: 'Category' }].map(({ key, label, required }) => (
                  <div key={key}>
                    ...
                    <input value={(faqForm as any)[key]} onChange={e => setFaqForm(p => ({ ...p, [key]: e.target.value }))} required={required} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white outline-none focus:border-purple-500" />
                  </div>
                ))}
                <div>
                  <label className="block text-sm text-white/60 mb-1">Answer</label>
                  <textarea rows={4} value={faqForm.answer} onChange={e => setFaqForm(p => ({ ...p, answer: e.target.value }))} required className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white outline-none focus:border-purple-500" />
                </div>
              ...
              <form onSubmit={handleClientSubmit} className="space-y-4">
                {[{ key: 'name', label: 'Client Name', required: true }, { key: 'logoUrl', label: 'Logo URL' }, { key: 'websiteUrl', label: 'Website URL' }].map(({ key, label, required }) => (
                  <div key={key}>
                    <label className="block text-sm text-white/60 mb-1">{label}</label>
                    <input value={(clientForm as any)[key]} onChange={e => setClientForm(p => ({ ...p, [key]: e.target.value }))} required={required} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white outline-none focus:border-purple-500" />
                  </div>
                ))}
                <div className="flex gap-3">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-2 rounded-lg bg-white/5 hover:bg-white/10">Cancel</button>
