    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="space-y-4">
        {isLoading ? <div className="p-8">Loading...</div> : 
         quotes.length === 0 ? <div className="p-8 glass-panel rounded-2xl text-white/50">No requests.</div> :
         quotes.map(q => (
           <div key={q.id} className="glass-panel p-6 rounded-2xl border-l-4 border-l-secondary">
             <div className="flex justify-between items-start mb-4">
               <div>
                 ...
                 <div className="text-sm text-white/60">From: {q.name} ({q.email}) • {q.phone || 'No phone'}</div>
               </div>
               <div className="text-right">
                 <div className="text-sm text-white/40 mb-2">{new Date(q.createdAt).toLocaleDateString()}</div>
                 <select 
                   value={q.status} 
                   onChange={(e) => handleUpdateStatus(q.id, e.target.value)}
                   className="bg-black/50 border border-white/10 rounded px-2 py-1 text-sm outline-none"
                 >
             ...
             <p className="text-white/80 whitespace-pre-wrap mb-4 bg-black/20 p-4 rounded-lg">{q.description}</p>
             <div className="flex justify-end">
               <button onClick={() => handleDelete(q.id)} className="px-3 py-1.5 bg-destructive/20 text-destructive hover:bg-destructive hover:text-white rounded text-sm flex items-center gap-1">
